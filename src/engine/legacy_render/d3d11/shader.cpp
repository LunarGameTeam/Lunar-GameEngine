...
void Dx11Shader::OnAssetFileLoad(LSharedPtr<AssetMetaData> meta, LSharedPtr<LFile> file)
{
	...
	result = device->GetD3DDevice()->CreateVertexShader(buffer->GetBufferPointer(), buffer->GetBufferSize(), NULL, &m_vertexShader);
	...
	result = device->GetD3DDevice()->CreatePixelShader(buffer->GetBufferPointer(), buffer->GetBufferSize(), NULL, &m_pixelShader);
	...
	result = device->GetD3DDevice()->CreateSamplerState(&samplerDesc, &m_sampleState);
}
