...
public:
	...
	virtual luna::LResult GetValueFromNode(const size_t &class_type_hash_in, void *variable_data_out) = 0;
	virtual luna::LResult SetValueToNode(const size_t &class_type_hash_in, const void *variable_data_in) = 0;
};
...
public:
	...
	luna::LResult GetValueFromNode(const size_t &class_type_hash_in, void *variable_data_out) override
	{
	...
	luna::LResult SetValueToNode(const size_t &class_type_hash_in, const void *variable_data_in) override
	{
		if (class_type_hash_in == m_class_type_hash)
		{
			//如果传入的参数类型一致，则直接赋值
...
class LReflectVariableCopySystem
{
	...
	std::unordered_map<size_t, luna::LResult(*)(const size_t &, void *, const void *)> m_type_translate_func_set;
	std::unordered_map<size_t, luna::LResult(*)(const size_t &, const void *, void *)> m_type_translate_func_get;
...
public:
	...
	luna::LResult GetValueFromPointer(const size_t deal_value_type, DataType &output_data, const void *pointer_data)
	{
		//先检查这个值的类型是否被注册过
		...
		luna::LResult check_error = deal_class->second->SetValueToNode(deal_value_type, pointer_data);
	...
	template<typename DataType>
	luna::LResult SetValueToPointer(const size_t deal_value_type, const DataType &input_data, void *pointer_data)
	{
		//先检查这个值的类型是否被注册过
		auto deal_class = all_variable_init.find(deal_value_type);
		if (deal_class == all_variable_init.end())
		...
		//如果注册过，则取出注册的类来处理这个变量的赋值拷贝
		luna::LResult check_error = deal_class->second->SetValueToNode(typeid(DataType).hash_code(), &input_data);
		if (!check_error.m_IsOK)
		{
			return check_error;
		}
	...
	template<typename DataType>
	luna::LResult GetArrayValueFromPointerByIndex(const size_t deal_value_type_in, const size_t &array_index, const void *pointer_data, DataType &output_data)
	{
	...
	template<typename DataType>
	luna::LResult SetArrayValueToPointerByIndex(const size_t deal_value_type_in, const size_t &array_index, void *pointer_data, const DataType &input_data)
	{
		luna::LResult check_error;
		//先检查类型是否是数组类型
		size_t deal_value_member_type;
	...
	//字符串处理枚举变量的信息
	luna::LResult SetEnumValueToPointer(const size_t &enum_type_id, const std::string &enum_name, const size_t deal_value_type_in, void *pointer_data);
	luna::LResult GetEnumValueFromPointer(const size_t &enum_type_id, std::string &enum_name, const void *pointer_data);
	luna::LResult SetEnumArrayValueToPointerByIndex(const size_t enum_type_id, const std::string &enum_name, const size_t &array_index, void *pointer_data);
	luna::LResult GetEnumArrayValueFromPointerByIndex(const size_t enum_type_id, std::string &enum_name, const size_t &array_index, const void *pointer_data);
...
private:
	...
	void InitArray(SerializeNodeType serialize_type)
	{
		...
		auto simple_array_lambda_get = [](const size_t &array_index, const void *array_data, void *reflect_member_out)->luna::LResult
		{
		...
		auto simple_array_lambda_set = [](const size_t &array_index, void *array_data, const void *reflect_member_out)->luna::LResult
		{
			//这里因为是注册赋值函数，并不能确定指针的大小，需要上面的反射层严格保证不会出现越界的情况！！！！
		...
		m_type_translate_func_get.insert(std::pair<size_t, luna::LResult(*)(const size_t &, const void *, void *)>(typeid(VariableNodeType *).hash_code(), simple_array_lambda_get));
		m_type_translate_func_set.insert(std::pair<size_t, luna::LResult(*)(const size_t &, void *, const void *)>(typeid(VariableNodeType *).hash_code(), simple_array_lambda_set));
		...
		auto simple_stdvector_lambda_get = [](const size_t &array_index, const void *array_data, void *reflect_member_out)->luna::LResult
		{
		...
		auto simple_stdvector_lambda_set = [](const size_t &array_index, void *array_data, const void *reflect_member_out)->luna::LResult
		{
		...
		m_type_translate_func_get.insert(std::pair<size_t, luna::LResult(*)(const size_t &, const void *, void *)>(typeid(std::vector<VariableNodeType>).hash_code(), simple_stdvector_lambda_get));
		m_type_translate_func_set.insert(std::pair<size_t, luna::LResult(*)(const size_t &, void *, const void *)>(typeid(std::vector<VariableNodeType>).hash_code(), simple_stdvector_lambda_set));
		...
		auto simple_boostvector_lambda_get = [](const size_t &array_index, const void *array_data, void *reflect_member_out)->luna::LResult
		{
		...
		auto simple_boostvector_lambda_set = [](const size_t &array_index, void *array_data, const void *reflect_member_out)->luna::LResult
		{
		...
		m_type_translate_func_get.insert(std::pair<size_t, luna::LResult(*)(const size_t &, const void *, void *)>(typeid(boost::container::vector<VariableNodeType>).hash_code(), simple_boostvector_lambda_get));
		m_type_translate_func_set.insert(std::pair<size_t, luna::LResult(*)(const size_t &, void *, const void *)>(typeid(boost::container::vector<VariableNodeType>).hash_code(), simple_boostvector_lambda_set));
	...
	luna::LResult GetEnumValueFromName(const size_t &enum_type_id, const std::string &enum_name, LEnumMidType &enum_value);
	luna::LResult GetEnumNameFromValue(const size_t &enum_type_id, std::string &enum_name, const LEnumMidType &enum_value);
};
...
public:
	...
	template<typename DataType>
	luna::LResult GetSingleValueFromNode(DataType &output_data) const
	{
		luna::LResult check_error;
	...
	template<typename DataType>
	luna::LResult SetSingleValueToNode(const DataType &input_data)
	{
		luna::LResult check_error;
	...
	template<typename DataType>
	luna::LResult GetArrayValueFromNode(DataType &output_data, const int32_t &index)const
	{
		luna::LResult check_error;
		//先检查最大值是否超过了数组范围
	...
	template<typename DataType>
	luna::LResult SetArrayValueToNode(const DataType &input_data, const int32_t &index)
	{
		//先检查最大值是否超过了数组范围
		luna::LResult check_error;
		if (index >= m_array_size)
	...
	//枚举变量
	luna::LResult GetSingleEnumValueFromNode(std::string &output_enum_name) const;
	luna::LResult SetSingleEnumValueToNode(const std::string &input_enum_name);
	luna::LResult GetArrayEnumValueFromNode(std::string &output_enum_name, const int32_t &index) const;
	luna::LResult SetArrayEnumValueToNode(const std::string &output_enum_name, const int32_t &index);
};
...
public:
	...
	void Create();
	//从json文件中加载数据
	luna::LResult LoadFromJsonFile(const std::string &Json_file);
	//从json内存中加载数据
	luna::LResult LoadFromJsonMemory(const std::string &value_name, const Json::Value &root_value);
	//将类成员数据存储到json文件
	luna::LResult SaveToJsonFile(const std::string &json_name);
	//将类成员数据存储到json内存
	luna::LResult SaveToJsonMemory(Json::Value &root_value);
	//将类成员数据拷贝到指定指针
	virtual luna::LResult CopyValueToData(LReflectNodeBasic &dst_pointer) = 0;
	virtual luna::LResult CopyValueToArrayData(LReflectNodeBasic &dst_pointer, const int32_t &index_array) = 0;
	//将指定指针的数据拷贝到类成员数据
	virtual luna::LResult ResetValueByData(const LReflectNodeBasic &dst_pointer) = 0;
	virtual luna::LResult ResetValueByArrayData(const LReflectNodeBasic &dst_pointer, const int32_t &index_array) = 0;
	//获取当前的成员变量
	virtual luna::LResult GetMemberValue(void *value_data) = 0;
	//将指定指针的数据拷贝到类成员数据
	virtual luna::LResult SetMemberValue(const void *value_data) = 0;
private:
	//创建子节点映射
	void BuildChildValueMap();
	//从json节点中加载数据
	luna::LResult LoadFromJsonNode(const std::string &parent_name, const std::string &value_name, const Json::Value &root_value);
	//从json数组中加载数据
	luna::LResult LoadFromJsonArray(const int32_t &value_index, const Json::Value &root_value);
	//将数据写入到json节点
	luna::LResult SaveToJsonNode(const int32_t &now_value_index, Json::Value &root_value);
	//注册所有的反射变量
	...
	//设置数组变量
	luna::LResult SetArrayValue(LReflectNodeBasic &reflect_data, const Json::Value &now_child_value);
	//设置结构体数组变量
	luna::LResult SetNodeArrayValue(LReflectNodeBasic &reflect_data, const int32_t &offset_value, const Json::Value &now_child_value);
	//获取基础变量
	luna::LResult SaveSingleValueMemberToJson(const LReflectNodeBasic &reflect_data, Json::Value &root_value);
	//获取数组变量
	luna::LResult SaveArrayValueMemberToJson(const LReflectNodeBasic &reflect_data, Json::Value &root_value);
	//将变量的完整名称转化为基础名称
	std::string TranslateFullNameToRealName(const std::string &full_name);
	//获取数组数据的真实大小
	luna::LResult GetArrayDataSize(const LReflectNodeBasic &reflect_data, size_t &size_out);
	//分割逗号字符串
	void DivideStringForReflect(const char *name_input, std::vector<std::string> &data_out);
protected:
	template<typename VariableType>
	luna::LResult AddAnyVariable(
	...
	luna::LResult BindArraySizeValue(
	...
	luna::LResult AddReflectData(const std::string &node_name, LReflectNodeBasic &node_data);
	luna::LResult SetStringValue(LReflectNodeBasic &reflect_data, const std::string &string_value);
	luna::LResult SetStringArrayValue(LReflectNodeBasic &reflect_data, const std::string &string_value, const int32_t &index);
	virtual void ClearMemory() = 0;
	//绑定所有的静态数组的大小标记变量
	template <class NowDealedVariable, class NowDealedSizeVariable, class ...Args>
	luna::LResult BindAllStaticVariable(const char *variable_name, NowDealedVariable &head, NowDealedSizeVariable &head_size, Args &... rest)
	{
	...
	template <class NowDealedVariable, class NowDealedSizeVariable, class ...Args>
	luna::LResult BindAllStaticVariable(const std::vector<std::string> &variable_name, const int32_t now_index, NowDealedVariable &head, NowDealedSizeVariable &head_size, Args &... rest)
	{
		luna::LResult check_error;
	...
	luna::LResult BindAllStaticVariable(const std::vector<std::string> &variable_name, const int32_t now_index)
	{
	...
	luna::LResult AddAllVariable(const char *variable_name, NowDealedVariable &head, Args &... rest)
	{
		std::vector<std::string> all_variable_name;
		DivideStringForReflect(variable_name, all_variable_name);
	...
	template <class NowDealedVariable, class ...Args>
	luna::LResult AddAllVariable(const std::vector<std::string> &variable_name, const int32_t now_index, NowDealedVariable &head, Args &... rest)
	{
		luna::LResult check_error;
	...
	luna::LResult AddAllVariable(const std::vector<std::string> &variable_name, const int32_t now_index)
	{
...
public:
	...
	//将类成员数据拷贝到指定指针
	luna::LResult CopyValueToData(LReflectNodeBasic &dst_pointer) override
	{
	...
	luna::LResult CopyValueToArrayData(LReflectNodeBasic &dst_pointer, const int32_t &index_array) override
	{
	...
	//将指定指针的数据拷贝到类成员数据
	luna::LResult ResetValueByData(const LReflectNodeBasic &dst_pointer) override
	{
	...
	luna::LResult ResetValueByArrayData(const LReflectNodeBasic &dst_pointer, const int32_t &index_array) override
	{
	...
	//获取当前的成员变量
	virtual luna::LResult GetMemberValue(void *value_data) override
	{
	...
	//将指定指针的数据拷贝到类成员数据
	virtual luna::LResult SetMemberValue(const void *value_data)override
	{
...
public:
	...
	luna::LResult SerializeToJsonFile(const class_name &data, const std::string &file_name)
	{
		Json::Value now_json_value;
		luna::LResult check_error = SerializeToJsonMemory(data, now_json_value);
		if (!check_error.m_IsOK)
		{
			return check_error;
		}
	...
	template<typename class_name>
	luna::LResult SerializeToJsonMemory(const class_name &data, Json::Value &now_json_value)
	{
	...
	template<typename class_name>
	luna::LResult DeserializeFromJsonMemory(class_name &data, const Json::Value &now_json_value, const std::string &json_name)
	{
		size_t now_class_id = typeid(class_name).hash_code();
		auto check_class = reflect_map.find(now_class_id);
		if (check_class == reflect_map.end())
	...
	template<typename class_name>
	luna::LResult DeserializeFromJsonFile(class_name &data, const std::string &file_name)
	{
		Json::Value now_json_value;
		luna::LResult check_error = PancyJsonTool::GetInstance()->LoadJsonFile(file_name, now_json_value);
		if (!check_error.m_IsOK)
		{
			return check_error;
		}
