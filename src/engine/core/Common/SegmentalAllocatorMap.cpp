...
SegmentalMemberValue::SegmentalMemberValue(SegmentalAllocatorMap* allocator) :m_data_offset(0), m_data_size(0), if_created(false), m_allocator(allocator)
{
};
...
LResult SegmentalMemberValue::Create(const size_t& data_offset, const size_t& data_size)
{
	if (if_created)
	{
		return g_Succeed;
...
LResult SegmentalAllocatorMap::Create()
{
	...
	check_error = BuildNewSegmentalToMap(root_segmental, root_segmental_data);
	if (!check_error.m_IsOK)
	{
		return check_error;
	}
...
LResult SegmentalAllocatorMap::ReleaseSegmental(const size_t& segmental_index, const size_t& segmental_size)
{
	...
	if (before_list_containter == m_empty_segmental_by_offset_map.end())
	{
