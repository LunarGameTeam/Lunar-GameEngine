...
public:
	...
	static LunarEngine::LResult SingleCreate(HWND hwnd, HINSTANCE hinst)
	{
		...
		else
		{
			...
			LunarEngine::LResult check_error = pancy_input_pInstance->Init(hwnd, hinst);
			return check_error;
		}
	}
	...
	LunarEngine::LResult Init(HWND hwnd, HINSTANCE hinst);
private:
	LunarEngine::LResult DinputClear(HWND hwnd, DWORD keyboardCoopFlags, DWORD mouseCoopFlags);//初始化函数
};