...
uint32_t LunarEngine::MyCountMips(uint32_t width, uint32_t height)
{
	if (width == 0 || height == 0)
		return 0;
...
inline bool LunarEngine::CheckIfDepthStencil(DXGI_FORMAT fmt)
{
	switch (fmt)
	{
...
inline void LunarEngine::MyAdjustPlaneResource(
...
HRESULT LunarEngine::MyFillInitData(_In_ size_t width,
	_In_ size_t height,
	_In_ size_t depth,
...
LResult PancyBasicTexture::LoadResoureDataByDesc(const PancyCommonTextureDesc& resource_desc)
{
	LResult check_error;
...
LResult PancyBasicTexture::LoadResoureDataByOtherFile(const std::string& file_name, PancyCommonTextureDesc& resource_desc)
{
...
LResult PancyBasicTexture::LoadPictureFromFile(PancyCommonTextureDesc& new_texture_desc)
{
	LResult check_error;
...
LResult PancyBasicTexture::UpdateTextureResource(std::vector<D3D12_SUBRESOURCE_DATA>& subresources, const PancyCommonTextureDesc& texture_desc)
{
	LResult check_error;
	//先对待拷贝的资源进行组织
...
LResult PancyBasicTexture::BuildTextureResource(
...
LResult PancyBasicTexture::BuildEmptyPicture(const PancyCommonTextureDesc& texture_desc)
{
	LResult check_error;
	ComPtr<ID3D12Resource> resource_data;
...
LResult PancyBasicTexture::CaptureTextureDataToWindows(DirectX::ScratchImage* new_image)
{
...
LResult PancyBasicTexture::SaveTextureToFile(
...
ResourceBlockGpu* LunarEngine::GetTextureResourceData(VirtualResourcePointer& virtual_pointer, LResult& check_error)
{
	...
	{
		LResult error_message;
		LunarDebugLogError(E_FAIL, "the vertex resource is not a texture", error_message);
...
LResult LunarEngine::LoadDDSTextureResource(
...
LResult LunarEngine::BuildTextureResource(
	const std::string& name_resource_in,
...
void LunarEngine::InitTextureJsonReflect()
{
	...
	InitNewStructToReflection(PancyCommonTextureDesc);
}